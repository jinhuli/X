...
class portfolio(object):
    def __init__(self):
        ...
        self.position=pd.DataFrame(index=[[],[]],columns = ["position"])
    def initialize(self,start_date,code_list):
        self.nav=pd.DataFrame(data=[1],index=[start_date],columns = ["nav"])
        self.position = pd.DataFrame(data=np.zeros(len(code_list), 1),index=[[start_date],code_list], columns=["position"])
